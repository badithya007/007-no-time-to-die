...
def articles_to_records(articles: list[dict]) -> list[dict]:
    rows = []
...
def articles_to_dataframe(articles: list[dict]) -> pd.DataFrame:
    return pd.DataFrame(articles_to_records(articles))
...
def filter_articles(articles: list[dict], query: str) -> list[dict]:
    ...
    for article in articles:
        if (
            lower_query in (article.get("title") or "").lower()
            or lower_query in (article.get("description") or "").lower()
            or lower_query in (article.get("source") or "").lower()
            or lower_query in (article.get("author") or "").lower()
        ):
